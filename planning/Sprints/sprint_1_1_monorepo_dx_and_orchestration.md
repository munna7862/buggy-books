...
- **Technical Subtasks**:
  - [ ] Inspect root `package.json` `scripts.install:all`.
  - [ ] Append `&& cd ../playwright-e2e && npm install` to the install sequence.
- **Acceptance Criteria**:
  - [ ] Running `npm run install:all` installs `node_modules` in `backend/`, `frontend/`, and `playwright-e2e/`.
...
- **Technical Subtasks**:
  - [ ] Add `"test:unit": "concurrently \"npm run test:backend\" \"npm run test:frontend\""` to root `package.json`.
  - [ ] Add `"test:backend": "cd backend && npm test"`.
  - [ ] Add `"test:frontend": "cd frontend && npm test"`.
  - [ ] Add `"test:e2e": "cd playwright-e2e && npm test"`.
  - [ ] Add `"lint": "cd frontend && npm run lint"`.
  - [ ] Add `"typecheck": "concurrently \"cd backend && npm run build\" \"cd frontend && tsc -b\" \"cd playwright-e2e && npx tsc --noEmit\""`.
  - [ ] Add `"build": "cd backend && npm run build && cd ../frontend && npm run build"`.
- **Acceptance Criteria**:
  - [ ] `npm run typecheck` validates TypeScript across all 3 subprojects.
  - [ ] `npm run test:unit` executes both backend Jest and frontend Vitest suites concurrently.
  - [ ] `npm run build` compiles production distributions for backend and frontend.
...
| :--- | :--- | :--- | :--- |
| **Dev Technical Review** | Dev Architect | Inspect root package.json for clean syntax and no broken paths. | `[PENDING]` |
| **SDET Quality Gate** | SDET Architect | Verify `test:unit` and `test:e2e` execution streams. | `[PENDING]` |
| **PO Acceptance Gate** | Product Owner | Verify developer ergonomics and README updates. | `[PENDING]` |

---

## 4. Definition of Done (DoD) Checklist

- [ ] All user story tasks are complete.
- [ ] Root `package.json` contains verified scripts.
- [ ] `npm run install:all` successfully provisions all 3 packages.
- [ ] `npm run typecheck` passes with 0 compilation errors.
- [ ] `npm run test:unit` passes with 0 test failures.
- [ ] Changes committed to a feature branch with conventional commits.
- [ ] Handoff verified by Scrum Master for Sprint 1.2 kickoff.
