...
- **Technical Subtasks**:
  - [ ] Investigate `fs.promises.rename` failures on Windows in [storage.ts](file:///c:/BuggyBooks/buggy-books/backend/src/data/storage.ts#L220).
  ...
  - [ ] Replace single-slot `pendingWrite` in `Storage` class with a sequential promise-chained write queue (or mutex queue) to ensure all mutations flush in strict order.
  - [ ] Ensure [storage.test.ts](file:///c:/BuggyBooks/buggy-books/backend/src/__tests__/storage.test.ts) exercises 50 rapid sequential and concurrent writes.
- **Acceptance Criteria**:
  - [ ] Executing 50 concurrent writes in [storage.test.ts](file:///c:/BuggyBooks/buggy-books/backend/src/__tests__/storage.test.ts) produces zero `EPERM`, `EBUSY`, or file corruption errors.
  - [ ] Final persisted `db.json` contains the exact state of the latest completed write.
...
- **Technical Subtasks**:
  - [ ] Inspect [app.ts lines 80-92](file:///c:/BuggyBooks/buggy-books/backend/src/app.ts#L80-L92):
    ```typescript
    // VULNERABLE: matches any arbitrary app on onrender.com
    /^([a-z0-9-]+\.)*onrender\.com$/.test(hostname);
    ```
  - [ ] Replace the wildcard regex with an explicit origin check:
  ...
  - [ ] Add unit test in `backend/src/__tests__/api.test.ts` verifying that requests with `Origin: https://malicious-app.onrender.com` are rejected by CORS.
- **Acceptance Criteria**:
  - [ ] Requests from `https://buggy-books-fe.onrender.com` pass CORS with `Access-Control-Allow-Origin` and `Access-Control-Allow-Credentials: true`.
  - [ ] Requests from arbitrary origins like `https://attacker.onrender.com` fail CORS validation.
...
- **Story Points**: 2 SP (Medium)
- **Technical Subtasks**:
  - [ ] In [server.ts](file:///c:/BuggyBooks/buggy-books/backend/src/server.ts#L69-L88), capture the `setInterval` handle:
    ```typescript
    ...
    ```
  - [ ] Ensure that when `NODE_ENV === 'test'`, the simulation interval is disabled completely or mockable.
  - [ ] In `SessionStorageManager` in [storage.ts](file:///c:/BuggyBooks/buggy-books/backend/src/data/storage.ts):
...
- **Acceptance Criteria**:
  - [ ] Importing `server.ts` or running Jest suites does not leave unref-ed timers keeping the Node process alive.
  - [ ] Continuous generation of 2,000 test sessions does not cause memory leaks beyond the configured capacity ceiling.

---

## 3. Definition of Done & Quality Gates

- [ ] `performWrite` in `storage.ts` passes atomic persistence unit tests on both Windows and Linux without `EPERM` errors.
- [ ] Mutex queue ensures all sequential state updates are written to disk without dropped frames.
- [ ] CORS rejects arbitrary `onrender.com` subdomains, proven by automated security regression tests.
- [ ] `server.ts` unrefs background intervals, yielding zero open handle warnings in Jest.
- [ ] All 12 backend test suites (85 tests) pass cleanly.
