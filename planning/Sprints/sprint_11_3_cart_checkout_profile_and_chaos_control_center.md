...
- **Story Points**: 2 SP
- **Technical Subtasks**:
  - [x] Implement `mobile/src/context/CartContext.tsx`:
  ...
  - [x] Author unit tests in `mobile/src/__tests__/CartContext.test.tsx` verifying cart addition, quantity updates, and cart badge counter updates.
- **Acceptance Criteria**:
  - [x] Adding an item from `BookDetailScreen` updates the Cart badge count immediately.
  - [x] Removing an item updates the subtotal and fires `DELETE /api/cart/:bookId`.
  - [x] Cart unit tests pass cleanly via `npm test --workspace=mobile`.
...
- **Technical Subtasks**:
  ...
  - [x] Order confirmation modal / success screen displaying generated `orderId`.
- **Acceptance Criteria**:
  - [x] Valid order submission clears the cart and displays order confirmation details.
  - [x] Invalid card or missing required fields display immediate visual validation errors.
...
- **Technical Subtasks**:
  ...
  - [x] Security Champion (SEC) Audit: Verify 2MB size limit and file type filters prevent arbitrary uploads.
  - [x] Implement "Sign Out" button with confirmation alert.
- **Acceptance Criteria**:
  - [x] Uploaded avatar is named with the authenticated username on backend disk storage (leveraging Sprint 11.1 Multer update using `req.user?.username`).
  - [x] Successfully uploading a photo updates the avatar display immediately.
  - [x] Permission denials display a helpful user alert directing to device settings.
...
- **Technical Subtasks**:
  ...
  - [x] SDET Task: Document test cases in `specs/test_cases_catalog.md`:
...
- **Acceptance Criteria**:
  - [x] Adjusting sliders and tapping Save immediately affects subsequent backend requests.
  - [x] Tapping Reset restores default failure rates and clears test carts and orders.
  - [x] `specs/test_cases_catalog.md` updated with complete Phase 11 test specifications.

---

## 3. Definition of Done (DoD)

- [x] All 4 user stories implemented and integrated into the bottom tab navigator.
- [x] End-to-end shopping flow (Add to Cart -> Checkout -> Success) passes on both Android and iOS.
- [x] Avatar upload operates smoothly with both Camera and Photo Library inputs and links to authenticated username.
- [x] Chaos settings dynamically update backend behavior.
- [x] TypeScript compilation succeeds with zero errors (`npm run typecheck:mobile`).
- [x] Mobile unit tests pass cleanly (`npm test --workspace=mobile`).
