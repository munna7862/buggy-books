...
- **Technical Subtasks**:
  - [x] Author `performance/scenarios/ecommerce-journey.js` using k6 multi-scenario execution or probabilistic action branching:
  ...
  - [x] Implement realistic user think times via random sleep jitter (`sleep(0.5 + Math.random() * 1.5)`).
  - [x] Capture granular Trend metrics: `journey_browse_duration`, `journey_search_duration`, `journey_cart_duration`, `journey_checkout_duration`.
  - [x] Establish golden baseline `performance/baselines/baseline-journey.json`.
  - [x] Add `npm run test:perf:journey` to `performance/package.json`.
- **Acceptance Criteria**:
  - [x] `ecommerce-journey.js` executes 50 concurrent VUs over 2 minutes without unhandled exceptions or session cross-talk.
  - [x] Metrics for browsing, searching, cart mutation, and checkout are reported as distinct Trend percentiles in the HTML report.
...
- **Technical Subtasks**:
  - [x] Author `performance/k6/auth-stress.js`:
  ...
  - [x] Enforce SLA threshold: `auth_login_duration: ['p(95)<350', 'p(99)<700']` under 40 RPS.
  - [x] Author golden baseline `performance/baselines/baseline-auth.json`.
  - [x] Add `npm run test:perf:auth` to `performance/package.json`.
- **Acceptance Criteria**:
  - [x] Benchmark highlights maximum sustainable login throughput before bcrypt threadpool starvation degrades latency beyond 500ms.
  - [x] Baseline comparison verifies login performance regression gates.
...
- **Story Points**: 2 SP (Medium)
- **Technical Subtasks**:
  - [x] Author `performance/k6/checkout-stress.js`:
    ...
    - Checks that remaining requests fail gracefully with HTTP 400/409 (`"Conflict"` / `"Insufficient stock"`) rather than HTTP 500 crashes.
  - [x] Measure transactional duration (`checkout_duration`) and lock contention times.
  - [x] Establish golden baseline `performance/baselines/baseline-checkout.json`.
  - [x] Add `npm run test:perf:checkout` to `performance/package.json`.
- **Acceptance Criteria**:
  - [x] Concurrent checkout test cleanly differentiates between HTTP 200 (successful purchase) and HTTP 400/409 (stock exhausted), asserting that exactly 0 overselling occurred.
  - [x] Transaction lock contention is profiled and documented in the performance HTML dashboard.

---

## 3. Definition of Done & Quality Gates

- [x] All 3 new test scripts (`ecommerce-journey.js`, `auth-stress.js`, `checkout-stress.js`) execute reliably via `node performance/run-k6.js`.
- [x] Golden baselines (`baseline-journey.json`, `baseline-auth.json`, `baseline-checkout.json`) are committed to `performance/baselines/`.
- [x] New npm scripts (`test:perf:journey`, `test:perf:auth`, `test:perf:checkout`) pass in both local environments and containerized runners.
- [x] Concurrency tests verify strict zero-overselling integrity on inventory checkouts.
- [x] Generated HTML dashboard visualizes stateful operation latencies side-by-side with read-only endpoints.
...
- **Completed Story Points**: 5 SP (100% Velocity)
- **Primary Deliverables**:
  ...
  5. Updated `package.json` and `performance/package.json` performance test scripts.
  6. CI pipeline integration in `.github/workflows/ci.yml`.
