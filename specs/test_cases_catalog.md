...
| ID | Title | Description | Priority | Target Coverage | Tags | Covered |
|:---|:---|:---|:---|:---|:---|:---|
| **MOB_E2E_01** | Obfuscated Locators (Challenging Test Locators) | Verify that `LoginScreen`, `RegisterScreen`, and `CheckoutScreen` use obfuscated testIDs (`txt_usr_77`, `txt_pwd_99`, `txt_fn_55`, `txt_f1`, `txt_l1`, `txt_addr_88`, `txt_c99`) and catalog items use computed testIDs (`btn_item_${id}_add`). Assert that test scripts successfully locate elements via accessibility labels and chained selectors. | High | Obfuscated Native Selectors (MOB-B1) | `@mobile` `@anti-pattern` `@locators` | **Yes**<br>- Screens: `LoginScreen.tsx`, `RegisterScreen.tsx`, `CatalogScreen.tsx`, `CheckoutScreen.tsx`<br>- Unit Tests: `mobile/src/__tests__/AntiPatterns.test.tsx`<br>- Maestro: `mobile-automation/.maestro/flows/auth/01_login_success.yaml`<br>- Appium: `mobile-automation/src/specs/auth.e2e.spec.ts` |
| **MOB_E2E_02** | Soft Keyboard Occlusion on Checkout | Verify that `CheckoutScreen` omits `KeyboardAvoidingView` so focusing credit card inputs on portrait devices occludes the "Place Order" CTA button. Assert that automation scripts must dismiss the keyboard (`driver.hideKeyboard()`) or perform upward drag gestures to reveal the CTA. | High | Keyboard Occlusion Handling (MOB-B2) | `@mobile` `@anti-pattern` `@keyboard` | **Yes**<br>- Screen: `CheckoutScreen.tsx`<br>- Unit Tests: `mobile/src/__tests__/AntiPatterns.test.tsx`<br>- Maestro: `mobile-automation/.maestro/flows/checkout/05_checkout_retry_loop.yaml`<br>- Appium: `mobile-automation/src/specs/checkout_chaos.e2e.spec.ts` |
| **MOB_E2E_03** | Dynamic Asynchronous Add-to-Cart Delays | Verify that tapping "Add to Cart" simulates a randomized 500ms–3500ms client latency during which the button is disabled with an activity indicator. Assert that test automation must explicitly wait for the bottom tab badge count or confirmation banner rather than static sleep. | High | Dynamic Asynchronous Wait (MOB-B3) | `@mobile` `@anti-pattern` `@delay` | **Yes**<br>- Screens: `BookDetailScreen.tsx`, `CatalogScreen.tsx`<br>- Unit Tests: `mobile/src/__tests__/AntiPatterns.test.tsx`<br>- Maestro: `mobile-automation/.maestro/flows/catalog/04_add_to_cart_delay.yaml`<br>- Appium: `mobile-automation/src/specs/catalog.e2e.spec.ts` |
| **MOB_E2E_04** | Stochastic Gateway Timeout & In-Screen Retry | Verify that stochastic payment gateway timeouts (HTTP 500) render an in-screen error banner (`banner_checkout_error`) with a "Retry Payment" CTA button (`btn_retry_payment`) while preserving cart items. Assert that tapping retry initiates payment resubmission leading to order confirmation without rebuilding the cart. | Critical | Stochastic Chaos Recovery (MOB-B4) | `@mobile` `@chaos` `@checkout-retry` | **Yes**<br>- Screen: `CheckoutScreen.tsx`<br>- Unit Tests: `mobile/src/__tests__/Checkout.test.tsx`, `mobile/src/__tests__/AntiPatterns.test.tsx`<br>- Maestro: `mobile-automation/.maestro/flows/checkout/05_checkout_retry_loop.yaml`<br>- Appium: `mobile-automation/src/specs/checkout_chaos.e2e.spec.ts` |
| **MOB_E2E_05** | Simulated Network Interruption & Offline Banner | Verify that toggling simulated offline mode in `ChaosScreen` causes `apiClient` requests to immediately reject with `ECONNABORTED` and renders a floating top `OfflineBanner` (`offline_banner`). Assert that turning offline mode off restores normal HTTP connectivity. | Critical | Offline Simulation & Recovery (MOB-B5) | `@mobile` `@chaos` `@offline-mode` | **Yes**<br>- Api: `client.ts`<br>- Components: `OfflineBanner.tsx`<br>- Screens: `ChaosScreen.tsx`<br>- Unit Tests: `mobile/src/__tests__/AntiPatterns.test.tsx`, `mobile/src/__tests__/Chaos.test.tsx`<br>- Appium Page Object: `mobile-automation/src/screens/ChaosScreen.ts` |
| **MOB_E2E_06** | Landscape Orientation Layout Shifts | Verify that `app.json` has `"orientation": "default"`. When rotating device into landscape mode (`width > height`), assert that the layout shift styling applies, challenging automation to handle viewport re-orientation and scroll-into-view interactions. | Medium | Multi-Orientation Responsiveness (MOB-B6) | `@mobile` `@orientation` `@landscape` | **Yes**<br>- Config: `mobile/app.json`<br>- Screens: `CheckoutScreen.tsx`<br>- Unit Tests: `mobile/src/__tests__/AntiPatterns.test.tsx`<br>- Maestro: `mobile-automation/.maestro/flows/checkout/06_orientation_layout_glitch.yaml`<br>- Appium: `mobile-automation/src/specs/orientation_chaos.e2e.spec.ts` |


---
## 23. Mobile CI/CD Pipeline & Build Artifacts

*Sprint Source: [Sprint 12.3: GitHub Actions Mobile CI/CD Pipeline & Build Artifacts](file:///c:/BuggyBooks/buggy-books/planning/Sprints/sprint_12_3_github_actions_mobile_cicd_pipeline_and_build_artifacts.md)*

These test cases validate the mobile continuous integration, testing, packaging, and release delivery automation across GitHub Actions, headless Android emulation, Maestro flow execution, and release APK compilation.

### **Suite: Mobile CI/CD Pipeline & Build Artifacts**
| ID | Title | Description | Priority | Target Coverage | Tags | Covered |
|:---|:---|:---|:---|:---|:---|:---|
| **MOB_CI_01** | Stage 1 Mobile Quality Gate in Main CI | Verify that `.github/workflows/ci.yml` includes the `mobile-quality-gate` job in Stage 1 parallel execution. Assert that it runs `npm run lint` and `npm run typecheck` across `mobile`, executes `npm run test:ci` (Jest with coverage), produces `test-results.json` and `coverage/coverage-summary.json`, parses them into `github-step-summary.md` via `scripts/generate-test-summary.js mobile mobile`, and uploads the HTML coverage report artifact. | High | Main CI Mobile Quality Gate | `@mobile` `@ci` `@quality-gate` | **Yes**<br>- Workflow: `.github/workflows/ci.yml`<br>- Script: `scripts/generate-test-summary.js`<br>- Config: `mobile/package.json` |
| **MOB_CI_02** | Dedicated Mobile CI with Headless Android Emulator & Maestro | Verify that `.github/workflows/mobile-ci.yml` triggers on pushes and PRs touching mobile paths. Assert that it sets up Java 17 and Maestro CLI on `macos-latest`, starts the backend with `wait-on`, exports the offline JS bundle via `npx expo export --platform android`, runs `npx expo prebuild --platform android --no-install`, compiles a standalone debug APK via `./gradlew assembleDebug`, boots a hardware-accelerated headless Android emulator (API 34) with `adb reverse tcp:4000 tcp:4000`, installs the APK, and executes all `.maestro/flows/` test suites with JUnit reporting and test artifact archiving. | Critical | Headless Mobile CI & Maestro E2E | `@mobile` `@ci` `@maestro` `@emulator` | **Yes**<br>- Workflow: `.github/workflows/mobile-ci.yml`<br>- Flows: `mobile-automation/.maestro/flows/` |
| **MOB_CI_03** | Automated Standalone Release APK Compilation & GitHub Release Packaging | Verify that `mobile/eas.json` defines `development`, `preview` (standalone APK), and `production` profiles. Assert that `.github/workflows/mobile-release.yml` triggers upon pushing `v*` release tags, exports production JS assets, prebuilds native Android sources, builds a standalone signed release APK via `./gradlew assembleRelease`, and publishes the binary artifact directly to GitHub Releases with release notes via `softprops/action-gh-release`. | High | EAS Config & Release Packaging | `@mobile` `@release` `@apk` `@github-release` | **Yes**<br>- Workflow: `.github/workflows/mobile-release.yml`<br>- Config: `mobile/eas.json` |
