...
| ID | Title | Description | Priority | Target Coverage | Tags | Covered |
|:---|:---|:---|:---|:---|:---|:---|
| **MOB_CAT_01** | Initial Catalog Load & Two-Column Grid Display | Verify that upon navigating to `CatalogScreen`, the app requests `GET /api/books` and renders books in a responsive 2-column `FlatList`. Assert that each book card displays cover image thumbnail, title, author, formatted price (`$X.XX`), and rating/genre badge with accessible touchable feedback. | High | Catalog Grid & FlatList Rendering | `@mobile` `@catalog` `@ui` | **Yes**<br>- Screen: `mobile/src/screens/CatalogScreen.tsx`<br>- Tests: `mobile/src/__tests__/Catalog.test.tsx` |
| **MOB_CAT_02** | Catalog Search Filtering with Debounced Queries | Verify that typing queries into the search bar debounces requests (`~300ms`) and queries `GET /api/books?q=<query>`. Assert that the book grid dynamically updates to display filtered results matching the search term. | High | Search Bar & Debounce Handling | `@mobile` `@search` `@debounce` | **Yes**<br>- Screen: `mobile/src/screens/CatalogScreen.tsx`<br>- Tests: `mobile/src/__tests__/Catalog.test.tsx` |
| **MOB_CAT_03** | Catalog Pull-to-Refresh State Synchronization | Verify that triggering pull-to-refresh (`RefreshControl`) on `CatalogScreen` sets the refreshing indicator, re-fetches catalog data from `GET /api/books`, updates the list, and dismisses the refresh spinner upon resolution. | Medium | Pull-to-Refresh & Data Sync | `@mobile` `@pull-to-refresh` `@ux` | **Yes**<br>- Screen: `mobile/src/screens/CatalogScreen.tsx`<br>- Tests: `mobile/src/__tests__/Catalog.test.tsx` |
| **MOB_CAT_04** | Catalog Search No-Results Empty State | Verify that when a search query returns zero matching books, `CatalogScreen` displays an accessible empty state with a "No books found" message and an option to clear the search filter. | Medium | Empty State UI & Feedback | `@mobile` `@empty-state` `@search` | **Yes**<br>- Screen: `mobile/src/screens/CatalogScreen.tsx`<br>- Tests: `mobile/src/__tests__/Catalog.test.tsx` |
| **MOB_CAT_05** | Book Detail View, Metadata Display & Quantity Selector | Verify that tapping any book card in `CatalogScreen` navigates to `BookDetailScreen` passing book details or ID. Assert that `BookDetailScreen` renders large cover art, full description, ISBN, stock status, interactive quantity selector (`-` and `+` buttons), and an "Add to Cart" CTA button. | High | Book Detail Screen & Quantity Selector | `@mobile` `@detail` `@navigation` | **Yes**<br>- Screen: `mobile/src/screens/BookDetailScreen.tsx`<br>- Tests: `mobile/src/__tests__/Catalog.test.tsx` |
